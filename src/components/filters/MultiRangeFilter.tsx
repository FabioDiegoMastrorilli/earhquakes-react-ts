...
import { MultiRangeFilter as MultiRangeFilterType, MultiRangeFilterValue } from "../../slices/earthquakes.types";
...
export default function MultiRangeFilter({matchKey, type, value}: MultiRangeFilterType) {
    const {earthquakes} = useAppSelector((state) => state.earthquakes);
    const dispatch = useAppDispatch();

    const limits = useMemo(() => {
        const range = earthquakes?.map(earthquake => earthquake.properties[matchKey]) || 
            [
